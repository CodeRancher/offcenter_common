...
int main(int argc, char **argv)
{
	try {
		...
		std::cout << "AMQP Managed Example App!" << std::endl;

		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::ToFile, "true");
		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::ToStandardOutput, "false");
		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Format, "%datetime:%thread [%level] %msg");
		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Filename, "./AmqpManaged.log");

		LOG(INFO) << "AMQP Managed Example App!";

		using App = amqpserver::AmqpManagedServerAppManager<AmqpManagedExampleApp>;
