...
int main(int argc, char **argv)
{
	try {
		...
		std::cout << "AMQP Manager Example App!" << std::endl;

		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::ToFile, "true");
		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::ToStandardOutput, "false");
		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Format, "%datetime:%thread [%level] %msg");
		el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Filename, "./AmqpManager.log");

		LOG(INFO) << "AMQP Manager Example App!";

		using App = amqpserver::AmqpServerAppManager<AmqpManagerExampleApp>;
